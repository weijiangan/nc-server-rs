...
> **Note (16.11 — Imaginary enablement resumed, 2026-08-09):** both parked complications
> (a)/(b) above are resolved and the task's **first** option — one Imaginary for both
> instances, bytes comparable — is now exercised:
> - `scripts/enable-preview-imaginary.sh` rewritten to configure **both** `nextcloud` and
>   `oracle` via `occ config:system:set` (complication (b) — config injection): identical
>   `enabledPreviewProviders` (Imaginary + MP3/TXT/MarkDown/OpenDocument/Krita; GD image
>   providers excluded — complication (a)) and `preview_imaginary_url =
>   http://previews_hpb:8088`, then restarts both (the SUT's Rust nc-server reads
>   config.php at startup). `previews_hpb` is now started in the compose bring-up.
> - **New harness surface: `compare_bytes`.** The `get` op gained a `compare_bytes` flag
>   capturing the raw response bytes (binary previews; mutually exclusive with
>   `compare_body`); the runner reports per-op "bytes identical (N B)" and a mismatch
>   fails the run. Scenario `27_imaginary_preview` (PUT 320×200 PNG → 64×64 GET miss →
>   64×64 GET hit → 128×128 GET, all `compare_bytes`) exercises the SUT's **native**
>   generation (Rust `preview_gen` → ImaginaryClient, log-verified
>   `native max/derived preview generated` + `previews_hpb:8088` connection) against
>   PHP `Imaginary.php`.
> - **Byte determinism grounded (live):** libvips is deterministic; a delete-preview-state
>   probe regenerated byte-identical output on both sides, and Rust-native vs PHP
>   generated bytes were `cmp`-clean. Caveat: compose pins `aio-imaginary:latest`
>   (floating) — an image upgrade between runs can change bytes across days, never within
>   a run (both sides hit the same instance).
> - **Result:** `27_imaginary_preview` → **IDENTICAL on both runs** (statuses, all three
>   byte comparisons, DB + fs deltas; only the accepted root-size entries). The preview
>   surface is fully green with Imaginary up — the SUT's native ops mirror
>   `Imaginary.php`'s pipeline byte-for-byte at three buckets (max 320×200, 64×64 crop,
>   128×128 crop).
> - **Finding #28's image clause does not reproduce (2026-08-09):** the live oracle writes
>   `creation_time = 0, upload_time = now` for a **plain** `image/png` PUT (same as text
>   and zip; query on both DBs for the probe file — identical rows), contradicting the
>   recorded "image → `creation_time = now`" matrix. The original observation used
>   `X-OC-Mtime`; the discriminator may have been the header, not the mimetype — the fix
>   phase should re-probe before trusting the #28 record. The differential shows parity
>   either way.
...
### 2026-08-09 — Imaginary enablement resumed: preview parity with byte comparison (`27_imaginary_preview`)

The parked Imaginary enablement (16.11 deviation 1) is resumed and the preview
surface is now fully green with the SUT generating **natively** through the same
Imaginary both sides share.

- **`scripts/enable-preview-imaginary.sh` rewritten** (per user direction: OCC-based,
  run from both instances — not the file-based approach first attempted, which was
  dropped). It now runs the `occ config:system:set` calls on **both** `nextcloud` and
  `oracle` (identical `enabledPreviewProviders`: Imaginary + MP3/TXT/MarkDown/
  OpenDocument/Krita — GD image providers excluded so Imaginary is the only image
  generator; `preview_imaginary_url = http://previews_hpb:8088`), then restarts both so
  the SUT's Rust nc-server (config read at startup) picks it up. `docs/services/
  imaginary.md` updated to the new flow.
- **`previews_hpb`** (aio-imaginary, `:8088`, `-return-size` + `-max-allowed-resolution`
  per the image's own args) is up; both containers reach it (`/health` 200).
- **New harness surface `compare_bytes`** (scenario.rs + runner): the `get` op captures
  raw response bytes (mutually exclusive with `compare_body`); the verdict reports
  "bytes identical (N B)" per op and a mismatch fails the run. Needed because
  `compare_body`'s lossy-text path is a parity signal but not byte-exact for binary
  previews.
- **Scenario `27_imaginary_preview`** authored (PUT 320×200 PNG → 64×64 GET miss →
  64×64 GET hit → 128×128 GET, all `compare_bytes`) and registered in divergences.yaml
  (inherits the PUT surface's `home-root-size` + `replay-second-boundary` entries).
- **Byte determinism grounded:** delete-preview-state-and-regenerate probe → byte-identical
  output on both sides; SUT-native (Rust `preview_gen` → ImaginaryClient, log-verified)
  vs PHP `Imaginary.php` → `cmp`-clean (238 B / 682 B). Caveat: `aio-imaginary:latest`
  floats — cross-day CI runs can change bytes, never within a run.
- **Result:** `27_imaginary_preview` → **IDENTICAL on runs 1 and 2** (statuses, all three
  byte comparisons, DB delta = the two accepted root-size entries only, fs delta clean).
  `25_preview_image` and `26_preview_unpreviewable` stay **IDENTICAL** (the negative case
  with Imaginary up: zip → 404 == 404, zero preview rows). 15/15 nc-difftest lib tests
  pass.
- **Finding #28 corrected:** the recorded "image PUT → `creation_time = now`" clause does
  not reproduce — the live oracle writes `creation_time = 0, upload_time = now` for a
  plain image PUT (verified on both DBs, identical rows). The original probe used
  `X-OC-Mtime`; the discriminator may have been the header, not the mimetype. Fix phase
  should re-probe before trusting the record; the differential shows parity either way.
