...
- [ ] In `build_pool`: add `.test_before_acquire(false)` to `AnyPoolOptions` (`pool.rs:22-27`); the pool relies on `max_lifetime`/`idle_timeout` for dead-connection reaping.
- [ ] Change `max_connections` 50 → 24 (`pool.rs:20`) — 2× the host's 12 cores; a Rust server can saturate Postgres, 50 backends thrash. (Value verified tunable via the S0 bench; do not lower `min_connections(5)`.)
- [ ] Add `pub fn backend_is_postgres() -> bool` (OnceLock set from `config.dbtype` in `build_pool`; default `false`). Consumed by 21.3 — land it here so S2's dialect branch has its one cached check.
