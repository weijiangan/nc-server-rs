...
- 2026-08-14: **22.2 implemented** — design notes: the tag sub-select is always-on in the CTE (one statement text; ungatable without a second SQL variant — when tags weren't requested it costs indexed lookups only, no statement). Empty-directory edge: no CTE rows → `dir_tags` absent → read_dir skips the dir cache insert (the target's `get_props` already cached it). Budgets lowered: `propfind_depth1` 14 → 13 (the delta per the gate). Verified: compiles, 311 + 38 lib tests green; milestone suite + perf-gate re-measure pending.
