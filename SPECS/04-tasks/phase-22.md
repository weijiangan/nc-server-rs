...
- 2026-08-14: **22.2 implemented** — superseded on the gating question by 22.2-C (below): the tag sub-selects are `CASE`-gated like the other four families, not always-on. Empty-directory edge: no CTE rows → `dir_tags` absent → read_dir skips the dir cache insert (the target's `get_props` already cached it). Budgets lowered: `propfind_depth1` 14 → 13 (the delta per the gate). Verified: compiles, 311 + 38 lib tests green; milestone suite + perf-gate re-measure pending.
