...
- 2026-08-14: **22.5/22.6 extended — per-node prop AVAILABILITY assertion + `05_propfind_web`.** The web files app's propfind A/B (production capture, cloud2.home.lan) exposed availability gaps the cardinality+hrefs check cannot catch: `nc:reminder-due-date` (PHP 200-empty vs Rust 404) and `d:quota-available-bytes` on files (PHP 404 vs Rust 200) — both fixed (b57fd3a), plus `nc:metadata-*` from `oc_files_metadata.json` (the blurhash the web app renders as the image preview placeholder — previously 404/blank; a CASE-gated CTE family at $10, text-column-as-String decode) and the rich-workspace pair (957f014). `PropfindShape` now carries per-node 200/404 prop sets; the parser fights found en route: closing tags yielded empty names (the `/` was in the scan's stop set — depth bookkeeping skipped, props lost), and the propstat's `<d:status>` comes AFTER its `<d:prop>` block (props must buffer until the status is seen). The assertion immediately proved PHP's allprop answers the standard DAV set ONLY — driving the vendored ALLPROP_STR shrink + the allprop driver-props exclusion + the dir-only quota gating (f754157/9fc6884): Rust's allprop now emits exactly PHP's set (getetag, getlastmodified, getcontentlength/type on files, resourcetype, quota-available-bytes/-used-bytes on dirs) with the `-3` SPACE_UNLIMITED sentinel from the vendored handler when the fs reports no total. All 7 propfind/write scenarios IDENTICAL with the assertion active; 313 nc-dav + 19 nc-difftest tests.
