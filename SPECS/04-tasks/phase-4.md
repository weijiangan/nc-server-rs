...
- [x] **Delegated to PHP-FPM.** SEARCH is not on the sync hot path — it's a UI action (user types in the search box), fires at human speed. The PHP `SearchDAV` + `FileSearchBackend` implementation is mature and complete (XML parsing, operator translation, SQL generation, DB query, 207 formatting — ~560 lines). A Rust reimplementation would be ~400+ lines of non-trivial XML→SQL translation for zero performance benefit.
- [x] **Implementation:** `router.rs` adds a `dav_arbiter_handler` wrapper at `/dav`, `/dav/`, `/remote.php/dav`, `/remote.php/dav/` that intercepts the SEARCH HTTP method and proxies to PHP-FPM via the existing FastCGI client. All other methods delegate to the native `dav_handler`. When FastCGI is not configured (`fastcgi_socket` absent from config), SEARCH falls through to the native handler which returns `405 Method Not Allowed`.
- [x] **No Rust SEARCH logic.** Zero code for XML parsing, operator translation, SQL generation, or 207 formatting. The entire pipeline runs in PHP: XML → SQL → query → response.

**Verify:** `SEARCH /remote.php/dav` with a `{DAV:}basicsearch` body scoped to `/files/{userId}` → returned by PHP-FPM with full `207 Multi-Status` response. If FastCGI is configured, this works out of the box.
