...
- [x] Merge the three `from_fn_with_state` layers (static → maintenance → auth) on the request stack into one composite (`router::http_middleware_stack`), each check refactored to return `Option<Response>`/`Result<_, Response>` with byte-identical early returns. Removes ~2 wrapper polls per await per layer (one wrapper chain instead of three).
...
- **2026-08-10 — 18.4: middleware consolidation (round-2 Task 4, 18.6).** The three `from_fn_with_state` layers (try_static_files → maintenance_guard → auth_layer) became one composite `router::http_middleware_stack`. Each middleware was refactored into a check returning `Option<Response>` / `Result<Vec<String>, Response>` (static: `None` = fall through; maintenance: `None` unless 503 applies, path passed as `&str`; auth: bypass paths return `Ok(vec![])`, rejections return `Err(response)`, the main path attaches `AuthInfo` and returns the session resolver's Set-Cookie values). The composite sequences them in the original order and appends the Set-Cookie headers after the handler — byte-identical responses. Two compile-level findings worth recording: (1) `from_fn` requires the middleware future to be `Send`, and the maintenance check initially took `&Request<Body>` — not `Send` because the body type is not `Sync` — fixed by passing the path string instead; (2) the async-fn `FnMut`/`Send` bounds surface as an opaque "FromFn trait not satisfied" error when the future is not Send. Verified: 12 nc-server tests (incl. the 4 middleware tests, now registering the composite), 20/20 diff-test on the first run, static probes `/core/img/actions/add.svg` → 200, `robots.txt` → 200, depth-1 PROPFIND 941 req/s / 3.65 ms p50 (noise band).
- **2026-08-15 — Static whitelist prefixes are now config-derived (memories `/wapps` incident).** The hardcoded list (`/core/ /dist/ /themes/ /apps/` + 2 root files) was verified only against the dev webroot; the live install runs memories from `/wapps` — its page HTML emits `src="/wapps/memories/js/memories-main.js?v=…"` via `getAppWebPath` → `apps_paths` `url` (`AppManager.php:729`, `base.php:157-175`) — and with nginx lacking static locations every asset request fell to Rust, where `/wapps/` was outside the whitelist → empty axum 404 (0-byte body). **Fix**: `try_static_files_check` now matches `AppState::static_prefixes`, derived once at startup by `router::static_prefixes_from_config(cfg, nc_root)`: fixed `/core/ /dist/ /themes/` + every `apps_paths` entry `url` rtrimmed of `/` (PHP `base.php:161`), with PHP's default-root fallback (`/apps`) gated on the directory existing (`base.php:167-170`). Per-request cost unchanged — a few `starts_with`, fs stat only on a candidate, so no regression on the DAV hot path. **Deliberate guards** (deviations, documented in the code): a webroot-root `url` (`/` or empty after rtrim) is skipped — as a prefix it would make every request path a static candidate, resurrecting the per-request fs stat Phase 18.1 removed; a `url` not starting with `/` is skipped (browser-relative — broken in PHP too); duplicates collapse; entries lacking `url`/`path` are skipped like PHP's `isset` checks. **Verified** on the dev SUT (config has four `apps_paths`: `/apps /apps-extra /apps-shared /apps-writable`): `/apps-extra/circles/css/dashboard-g12OwVzn.chunk.css` 404 → **200** (4.2 KB); deny side unchanged (`/data/.ocdata` 404, `/apps-extra/` 404, `robots.txt` 200, `/core/img/actions/add.svg` 200). Tests: 7 derivation unit tests (`router.rs`), `static_serves_custom_app_dir` + `static_denies_unlisted_app_dir` integration pins (`auth.rs`), `apps_paths_parses` (`nc-db`); 50 nc-server + 39 nc-db green. Known limitation (pre-existing): the fallback PHP-array parser still cannot parse nested arrays — a config with `apps_paths` requires the PHP-CLI loader (the primary path). Deployment note: live nginx now serves statics per the canonical template, so Rust's static layer is dormant there; this change is what standalone mode needs.
- **2026-08-16 — The fall-through assumption was disproven by a live capture; the arbiter's default branch now proxies to PHP.** Governing decision (above): *"`dav_handler` already resolves its own strip prefix, so the fall-through is unchanged"* — wrong. The live Photos app's Places view PROPFINDs `remote.php/dav/photos/{uid}/places/` (Depth 1, `nc:last-photo`/`nc:nbItems`); PHP serves it — the photos app registers `photos` + `photospublic` sabre collections in `appinfo/info.xml` (`<sabre><collections>`), mounted into the root by `apps/dav/lib/Server.php:415-417` — but the fall-through dropped the path into the native files tree, which 404s everything that is not a filecache path (empty-body 404, reproduced header-for-header on the dev SUT against the live HAR). **Fix** (`router.rs`): `dav_arbiter_handler` now classifies `/files` (the hot path) explicitly native and proxies everything else to PHP-FPM (native fallback when `fastcgi` is unset). This also covers the fixed root children the static list missed (`systemtags`, `provisioning`) and any future app collection — PHP is the registrar of its own root, so a static list can never be complete. The proxied-10 + SEARCH/REPORT + uploads/bulk classification is unchanged; the route table stays 6 routes, so the Phase 18 clone-cost win is untouched. Verified: dev :8080 replay of the exact PROPFIND is byte-identical to the live response (same `dav:` header, `content-length: 0`, CSP, `x-request-id`); PHP's own sabre 404 on :9090 (dev has no photos app) carries the XML error body instead.
