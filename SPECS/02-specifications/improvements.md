...
**Rust behavior (flat override):** for media uploads (`image/*` or `video/*`) where the client sent a valid `X-OC-CTime`, that value becomes the effective mtime **unconditionally** (written to `oc_filecache.mtime`/`storage_mtime`, the **disk** file mtime on both upload paths, propagation, version rows — everything the original mtime would feed). No arrival-window check, no "both headers" requirement, no mtime/ctime ordering constraint. `false` → strict PHP semantics.

The disk mtime matters for scan survival: a later `occ files:scan` compares the disk-derived `storage_mtime` against the cached one (`Scanner.php:173`) and only keeps the cached mtime when they match. PHP's simple PUT always touches the disk file (`File.php:351` `View::touch` → `Local::touch`); the Rust simple-PUT path now does the same (`davfile.rs` flush `set_file_times`), so the override is not reverted by a rescan.
