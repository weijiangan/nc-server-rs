...
- **Cargo cache corruption in image builds**: a network-EOF'd download can leave corrupt extractions in the `cargo-registry` cache mount; the build then fails with `no targets specified in the manifest` at a *random* crate. Fix surgically: a one-off `podman build` whose `RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry` does `rm -rf …/registry/src` (cargo re-extracts from the intact `.crate` files — no re-download).
- **Hot-swapping a host-built `nc-server` into the container drops its `cap_net_bind_service` xattr** (baked by `Dockerfile:148`); since `bootstrap.sh` runs nc-server as `www-data`, it then dies with `Failed to bind to 0.0.0.0:80 — Permission denied`. After any `docker cp` of the binary, re-apply `docker exec master-nextcloud-1 setcap cap_net_bind_service=+ep /usr/local/bin/nc-server` and restart.
