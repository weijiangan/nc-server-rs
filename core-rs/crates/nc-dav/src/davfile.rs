...
pub struct WriteCtx {
    ...
    /// media-mtime override (improvements.md) applies to media only.
    pub is_media: bool,
    /// Config `media_mtime_ctime_fallback` (improvements.md) — the media-mtime
    /// override switch.  Off → strict PHP semantics.
...
impl DavFile for NcDavFile {
    ...
    fn flush(&'_ mut self) -> FsFuture<'_, ()> {
        async move {
            ...
            // §10.5 + improvements.md: the flat media-mtime override.  For
            // media uploads the client's X-OC-CTime — the capture/save date —
            // becomes the effective mtime unconditionally, so the iOS client's
            // Photos tab (sorted by d:getlastmodified) places them on the day
            // they were taken/saved, not the upload day.  (The windowed
            // heuristic of commit 0563441 missed deferred background uploads.)
            let use_mtime = crate::mtime::media_mtime_ctime_override(
                use_mtime,
                ctx.x_oc_ctime,
                ctx.is_media,
